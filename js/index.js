...
function renderPrice() {
  ...
  const prices = document.querySelectorAll('aside.price li');
  let totalPrice = 10;

  if (!state.pepperoni) prices[0].style.display = 'none';
  else {
    prices[0].style.display = 'list-item';
    totalPrice += 1;
  }
  if (!state.mushrooms) prices[1].style.display = 'none';
  else {
    prices[1].style.display = 'list-item';
    totalPrice += 1;
  }
  if (!state.greenPeppers) prices[2].style.display = 'none';
  else {
    prices[2].style.display = 'list-item';
    totalPrice += 1;
  }
  if (!state.whiteSauce) prices[3].style.display = 'none';
  else {
    prices[3].style.display = 'list-item';
    totalPrice += 3;
  }
  if (!state.glutenFreeCrust) prices[4].style.display = 'none';
  else {
    prices[4].style.display = 'list-item';
    totalPrice += 5;
  }
  
  document.querySelector('aside.price strong').innerHTML = "$" + totalPrice;
}
